...
This document describes all new features added to make c4r more user-friendly.
...
**Builder Functions:**
- `c4_builder()` - Initialize builder
- `set_title()`, `set_theme()` - Set properties
- `add_person()`, `add_system()`, `add_container()`, `add_component()`, `add_external_system()` - Add elements
- `add_relationship()` - Add relationships
- `add_group()` - Add visual grouping
- `build_context()`, `build_container()`, `build_component()` - Build diagram
...
**Types:** `"sync"`, `"async"`, `"request-response"`
...
**Note:** PNG, SVG, and PDF export require `DiagrammeRsvg` and `rsvg` packages.
...
See `inst/examples/comprehensive_example.R` for a complete working example demonstrating all features.
...
- Interactive diagrams (Shiny integration)
- Diagram comparison and versioning
- Import from external formats (Structurizr, PlantUML)
- RStudio Addin
- And more!
